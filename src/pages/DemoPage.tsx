    ...
    <div className="min-h-screen bg-background px-4 pb-6 pt-4">
      ...
      >
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <button className="btn-ghost inline-flex items-center gap-2" onClick={() => { setFrameFailed(false); setFrameKey((k) => k + 1) }}><RefreshCcw className="h-4 w-4" /> Reload demo</button>
            <a className="btn-ghost inline-flex items-center gap-2" href={demoPath} target="_blank" rel="noreferrer"><ExternalLink className="h-4 w-4" /> Open full page</a>
      ...
      {exists && !frameFailed ? (
        ...
        <div className="glass rounded-2xl p-6 text-sm">
          Demo preview could not load. Open the standalone demo page instead: <a className="text-[#00e5b4] hover:underline" href={demoPath} target="_blank" rel="noreferrer">{demoPath}</a>
        </div>
      )}
      </div>
    </div>
